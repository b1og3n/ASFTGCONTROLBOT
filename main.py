...
def format_bot_ui(bot: dict) -> str:
    ...
    if idle_games:
        ...
        if bot.get("IsConnectedAndLoggedOn"):
            idle_text = f"Idle: включен ({', '.join(names)})"
        else:
            idle_text = f"Idle: включен ({', '.join(names)}) применится после запуска"
            text += f"{idle_text}\n"
    else:
        text += "Idle: выключен\n"
    text += (
...
def get_asf_status_text():
    response = requests.get(ASF_URL, headers={"Authentication": ASF_PASSWORD})
...
async def get_confirmations(bot_name: str):
    async with aiohttp.ClientSession() as session:
        async with session.get(f"http://127.0.0.1:1242/Api/Bot/{bot_name}/TwoFactorAuthentication/Confirmations", headers={"Authentication": ASF_PASSWORD}) as response:
            if response.status != 200:
                return []
            data = await response.json()
            if not data.get("Success"):
...
async def get_inventory(bot_name: str, appid: int, contextid: int = 2):
    ...
    async with aiohttp.ClientSession() as session:
        async with session.get(url, headers={"Authentication": ASF_PASSWORD}) as response:
            if response.status != 200:
                return None
            data = await response.json()
            if not data.get("Success"):
...
async def render_inventory_page(callback: CallbackQuery, bot_name: str, inventory_type: str, appid: int, contextid: int, page: int):
    await callback.message.edit_text("Загрузка inventory...")
    inventory = await get_inventory(bot_name, appid, contextid)
    if not inventory:
        await callback.message.edit_text("Не удалось загрузить inventory")
        return
    bot_inventory = inventory.get(bot_name, {})
    assets = bot_inventory.get("Assets", [])
    descriptions = bot_inventory.get("Descriptions", [])
    if not assets:
        await callback.message.edit_text("Инвентарь пуст", reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="Назад", callback_data=f"inventory_{bot_name}")]]))
        return
    desc_map = {}
    ...
    await callback.message.edit_text(text[:4000], reply_markup=InlineKeyboardMarkup(inline_keyboard=keyboard))
...
def main_keyboard():
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="♻ Restart ASF", callback_data="restart_asf_confirm")],
    ])
...
async def console_enter(callback: CallbackQuery):
    ...
    await callback.message.edit_text("Консоль ASF\n\nОтправь команду (без !)", reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="Назад", callback_data="console_exit")]]))
...
async def bot_selected(callback: CallbackQuery):
    ...
    try:
        ...
        keyboard.append([InlineKeyboardButton(text="Игры", callback_data=f"games_{bot_name}")])
        ...
        await callback.message.edit_text(text, parse_mode="HTML", disable_web_page_preview=True, reply_markup=InlineKeyboardMarkup(inline_keyboard=keyboard))
    except Exception as e:
        await callback.message.edit_text(f"Ошибка: {e}")
...
async def games(callback: CallbackQuery):
    ...
    await callback.message.edit_text("Выберите игру для накрутки часов", reply_markup=games_keyboard(bot_name, idle_enabled))
...
async def restart_asf(callback: CallbackQuery):
    ...
    try:
        ...
        if response.status_code != 200:
            await callback.message.edit_text(f"Ошибка HTTP {response.status_code}", reply_markup=main_keyboard())
            return
        await asyncio.sleep(6)
        text = get_asf_status_text()
        await callback.message.edit_text(f"ASF успешно перезапущен\n\n{text}", reply_markup=main_keyboard())
    except Exception as e:
        await callback.message.edit_text(f"Ошибка: {e}", reply_markup=main_keyboard())
...
async def farm_game(callback: CallbackQuery):
    ...
    try:
        ...
        try:
            await callback.message.edit_reply_markup(reply_markup=games_keyboard(bot_name, app_id in idle_games))
        except:
            pass
    except Exception as e:
...
async def inventory_menu(callback: CallbackQuery):
    ...
    await callback.message.edit_text("Проверка inventory...")
    ...
    if not cs2_assets and not steam_assets:
        await callback.message.edit_text(f"📦 Инвентарь {bot_name} пуст", reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="Назад", callback_data=f"bot_{bot_name}")]]))
        return
    keyboard = []
    if cs2_assets:
...
async def inventory_page(callback: CallbackQuery):
    await callback.answer()
    _, inv_type, bot_name, page = callback.data.split("|")
    page = int(page)
    if inv_type == "cs2":
...
async def text_router(message: Message):
    # console mode
    if message.from_user.id in console_users:
        ...
        try:
            ...
            await message.answer(text, parse_mode="HTML", reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="Удалить", callback_data="delete_msg")]]))
        except Exception as e:
            await message.answer(f"Ошибка: {e}")
        return
    # single bot redeem mode
    if message.from_user.id in bot_redeem_users:
        bot_name = bot_redeem_users.pop(message.from_user.id)
        menu_message = bot_redeem_messages.pop(message.from_user.id, None)
        if menu_message:
            try:
                response = requests.get("http://127.0.0.1:1242/Api/Bot/ASF",headers={"Authentication": ASF_PASSWORD})
                data = response.json()
                    ...
                    await menu_message.edit_text(
                        ...
                        reply_markup=InlineKeyboardMarkup(
                            inline_keyboard=[
                                ...
                                [InlineKeyboardButton(text="Игры", callback_data=f"games_{bot_name}")],
        ...
        if len(text) > 4000:
            text = text[:4000] + "\n\n...обрезано"
        await checking.edit_text(text, parse_mode="HTML")
        return
    # redeem mode
    if message.from_user.id in redeem_users:
        redeem_users.discard(message.from_user.id)
        menu_message = redeem_messages.pop(message.from_user.id, None)
        if menu_message:
            try:
